...
# Layer class
class Layer:
    def __init__(self, num_nodes, num_inputs, activation_function):
        """
        Layer constructor
    ...
    def calculate_gradients(
        self, target_or_weights, layer_type, next_layer_deltas=None
    ):
        activator = ActivationFunction(self.activation_function)
        if layer_type == "output":
            # print("Output Layer")
            # target_or_weights = target values
            self.deltas = (target_or_weights - self.outputs) * activator.activate(
                self.outputs, derivative=True
            )
            ...
            self.deltas = hidden_errors * activator.activate(
                self.outputs, derivative=True
            )
